...
class AppController extends \BaseController {
	...
	public function getLocationInfo($id)
	{
		...
		$location_tag_id = $this->locationRepository->getLocationTag($id);	
		DB::setFetchMode(PDO::FETCH_ASSOC);
		$myTag = DB::table('tags')
                    ->where('id', '=', $location_tag_id[0]['location_tag_id'])->get();
		$parentTagId = $myTag[0]['parent_tag_id'];	
		DB::setFetchMode(PDO::FETCH_CLASS);
		$comments = $this->commentRepository->findByLocation($id);
		return View::make('app.layouts.location', compact('location', 'comments','images','parentTagId'));
	}
